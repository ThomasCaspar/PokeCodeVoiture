...
/*
 *  EssuieGlace CODE CREE POKECODE
 */
...
int led  = 0;
  ...
  (void)argv;
  if (argc != 1) {
    chprintf(chp, "Usage: EssuieGlace <(0/1)>\r\n");
    return;
  }
  ...
  if(led == 1)palSetPad(PORT_C,PC_LEDBLEU );
  else palClearPad(PORT_C,PC_LEDBLEU);

}
/*
 *  Light CODE CREE POKECODE
 */
...
int led  = 0;
  ...
  if(led == 1)palSetPad(PORT_B,PB_LEDBLANCHE );
  else palClearPad(PORT_B,PB_LEDBLANCHE);

}

/*
 *  StopLight CODE CREE POKECODE
 */
...
int led  = 0;
  ...
  (void)argv;
  if (argc != 1) {
    chprintf(chp, "Usage: StopLight<(0/1)>\r\n");
    return;
  }
  ...
  if(led == 1)palSetPad(PORT_C,PC_LEDROUGE );
...
/*
 * Array of defined commands, you can add more. CODE MODIFIE POKECODE
 */
