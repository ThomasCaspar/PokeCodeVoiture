...
#if !defined(_CHIBIOS_RT_) || (!CH_USE_QUEUES && !CH_CFG_USE_QUEUES) ||     \
    defined(__DOXYGEN__)
...
typedef GenericQueue io_queue_t;
typedef InputQueue input_queue_t;
typedef OutputQueue output_queue_t;
#endif
...
#define iqObjectInit(iqp, bp, size, infy, link)                             \
  chIQInit(iqp, bp, size, infy, link)
#else
#define iqObjectInit(iqp, bp, size, infy, link)                             \
...
#define oqObjectInit(oqp, bp, size, onfy, link)                             \
  chOQInit(oqp, bp, size, onfy, link)
#else
#define oqObjectInit(oqp, bp, size, onfy, link)                             \
